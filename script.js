/* =============================================
   JERUDONG HEAVY CROP SHEARS DEPOT
...
(function initHeroScene() {"
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const color = new THREE.Color(Math.random() > 0.4 ? 0xffffff : 0x15803d);
    colors[i3] = color.r;
    colors[i3 + 1] = color.g;
    colors[i3 + 2] = color.b;
  ...
  // --- CUSTOM THREE.JS CUSTOM GEOMETRY INJECTION ---
    ...
    // Flat box and cylinder shapes representing crop shears blades rotating
    function createShearsBlade(color) {
      const group = new THREE.Group();
      const bladeGeo = new THREE.BoxGeometry(0.3, 2.5, 0.08);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      blades.push({
        ...
        baseY: blade.position.y
      });
    }
    scene.add(shearGroup);
  ...
  const dirLight1 = new THREE.DirectionalLight(0x15803d, 1.2);
  dirLight1.position.set(12, 18, 10);
  scene.add(dirLight1);

  const dirLight2 = new THREE.DirectionalLight(0xa16207, 0.8);
  dirLight2.position.set(-10, -5, -5);
  scene.add(dirLight2);

  const pointLight = new THREE.PointLight(0x15803d, 1.0, 45);
